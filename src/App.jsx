...
const PYTHON_SERVICE_URL = import.meta.env.VITE_PYTHON_SERVICE_URL || 'http://localhost:8080';
...
const Matchmaker = ({ db, userId, userName, userType, onStartConversation }) => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      const endpoint = `${API_URL}/smart-match-public`;
      ...
      if (err.name === 'AbortError' || err.message.includes('Failed to fetch') || err.message.includes('NetworkError')) {
        setError(`⚠️ Backend server is not responding. Please ensure the backend API is running and accessible at ${API_URL}`);
      } else if (err.message.includes('CORS')) {
        setError(`⚠️ CORS error detected. The backend may not be properly configured. Check backend CORS settings.`);
      } else {
...
const App = () => {
  ...
  const renderContactUsTab = () => {
    ...
    const handleContactSubmit = async (e) => {
      ...
      try {
        ...
        try {
          const emailResponse = await fetch(`${API_URL}/api/contact`, {
            method: 'POST',
            headers: {
            ...
            <div>
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    const response = await fetch(`${PYTHON_SERVICE_URL}/api/paper-analysis`, {
                      method: 'POST',
                      body: formData,
                    });
            ...
            <div>
              ...
              <button
                onClick={async () => {
                  ...
                  setMessage('');
                  try {
                    const response = await fetch(`${API_URL}/api/mentorship`, {
                      method: 'POST',
  ...
  const handleMultiAgentSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/multi-agent/mentorship`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
